...
//
//
//    SDL_Surface * image = IMG_Load("/res/whole-cabbage-kimchi.jpg");
//
...
//    IMG_Init(IMG_INIT_JPG);
    ...
    if(!window) {
        SDL_Log("Failed to make window\n");
    }
    SDL_Renderer * renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    SDL_Surface * image = IMG_Load("whole-cabbage-kimchi.jpg");
    if(!image) {
...
//    SDL_Surface * image = SDL_LoadBMP("hello.bmp");
    ...
    while (!quit)
    {
        ...
        if(ball.x < 0 && velx < 0) velx *= -1;

        if(ball.x > Width-ball.w && velx > 0 ) velx *= -1;

        if(ball.y < 0 && vely < 0) vely *= -1;

        if(ball.y > Height-ball.w && vely > 0 ) vely *= -1;
